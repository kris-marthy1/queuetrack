...
{remainingRows.length > 0 ? (
  <TableContainer component={Paper} sx={{overflowX: 'auto', width: '100%'}}>
    <Table aria-label="customized table">
      <TableHead>
        <TableRow>
          {['queue_id', 'priority'].map((column) => (
            <StyledTableCell key={column} sx={{whiteSpace: 'nowrap'}}>
              {column.replace(/_/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase())}
            </StyledTableCell>
          ))}
          <StyledTableCell sx={{whiteSpace: 'nowrap'}}>Estimated Duration</StyledTableCell>
        </TableRow>
      </TableHead>
      <TableBody>
        {remainingRows.map((row, index) => (
          <TableRow key={index} sx={{backgroundColor: queueSession?.queue_id?.toString() === row.queue_id.toString() ? 'lightyellow' : 'white'}}>
            {['queue_id', 'priority'].map((column) => (
              <TableCell key={column} sx={{whiteSpace: 'nowrap'}}>{row[column] || 'N/A'}</TableCell>
            ))}
            <TableCell sx={{whiteSpace: 'nowrap'}}>{estimatedTime?.estimated_duration ? estimatedTime?.estimated_duration * (index + 1) : 5 * (index + 1)} minutes</TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
